...
describe('Coordinator/Users', function() {
  ...
  describe('subscribeEvent', function() {
    context('when all the required parameters are provided', function() {
      let expectedUserEvent;
      let promise;
      let request;
      let event;
      let user;
      let userEventFromServer;
      let toCamelCase;
      ...
      beforeEach(function() {
        ...
        expectedUserEvent = fixture.build('contxtUserEvent');
        userEventFromServer = fixture.build(
          'contxtUserEvent',
          expectedUserEvent,
          {
            fromServer: true
        ...
        request = {
          ...baseRequest,
          post: sinon.stub().resolves(userEventFromServer)
        };
        toCamelCase = sinon
          .stub(objectUtils, 'toCamelCase')
          .callsFake((app) => expectedUserEvent);

        const users = new Users(baseSdk, request, expectedHost);
        promise = users.subscribeEvent(user.id, event.id);
      });
      ...
      it('formats the returned user event', function() {
        return promise.then(() => {
          expect(toCamelCase).to.be.calledWith(userEventFromServer);
        });
      });
    ...
    context('when the user ID is not provided', function() {
      it('throws an error', function() {
        ...
        const users = new Users(baseSdk, baseRequest, expectedHost);
        const promise = users.subscribeEvent(null, event.id);
    ...
    context('when the event ID is not provided', function() {
      it('throws an error', function() {
        ...
        const users = new Users(baseSdk, baseRequest, expectedHost);
        const promise = users.subscribeEvent(user.id, null);

        return expect(promise).to.be.rejectedWith(
          'An event ID is required for adding an event to a user'
        );
      });
    });
  });
  ...
  describe('unsubscribeEvent', function() {
    context('when all required parameters are provided', function() {
      ...
      beforeEach(function() {
        userEventSubscription = fixture.build('event');
        user = fixture.build('contxtUser');

        const users = new Users(baseSdk, baseRequest, expectedHost);
        promise = users.unsubscribeEvent(user.id, userEventSubscription.id);
      });
    ...
    context('when the user ID is not provided', function() {
      it('throws an error', function() {
        ...
        const users = new Users(baseSdk, baseRequest, expectedHost);
        const promise = users.unsubscribeEvent(null, userEventSubscription.id);
    ...
    context('when the application ID is not provided', function() {
      it('throws an error', function() {
        const user = fixture.build('contxtUser');
        const users = new Users(baseSdk, baseRequest, expectedHost);
        const promise = users.unsubscribeEvent(user.id, null);
